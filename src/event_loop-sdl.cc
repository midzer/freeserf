...
EventLoopSDL::run() {
    ...
    // --- Timing setup ---
    const int TICKS_PER_SECOND = 50;        // fixed update frequency
            ...
            switch (event.type) {
                case SDL_MOUSEBUTTONUP:
                    ...
                    if (event.button.button <= 3) {
                        ...
                        if (current - last_click[event.button.button] < MOUSE_TIME_SENSITIVITY &&
                        ...
                        last_click[event.button.button] = current;
                ...
                case SDL_KEYDOWN: {
                    ...
                    if (event.key.keysym.mod & KMOD_CTRL)  modifier |= 1;
                    if (event.key.keysym.mod & KMOD_SHIFT) modifier |= 2;
                    if (event.key.keysym.mod & KMOD_ALT)   modifier |= 4;

                    switch (event.key.keysym.sym) {
                        case SDLK_UP:    notify_drag(0,0, 0,-32,Event::ButtonLeft); break;
                        case SDLK_DOWN:  notify_drag(0,0, 0, 32,Event::ButtonLeft); break;
                        case SDLK_LEFT:  notify_drag(0,0,-32, 0,Event::ButtonLeft); break;
                        case SDLK_RIGHT: notify_drag(0,0, 32, 0,Event::ButtonLeft); break;
                        case SDLK_PLUS:
                        case SDLK_KP_PLUS:
                        case SDLK_EQUALS: notify_key_pressed('+', 0); break;
                        case SDLK_MINUS:
                        case SDLK_KP_MINUS: notify_key_pressed('-', 0); break;
                        ...
                        case SDLK_RIGHTBRACKET: zoom(-0.2f); break;
                        case SDLK_LEFTBRACKET:  zoom(0.2f); break;

                        case SDLK_F10: notify_key_pressed('n', 1); break;
                        default: notify_key_pressed(event.key.keysym.sym, modifier); break;
                    }
                    break;
                }
                ...
                case SDL_USEREVENT:
                    switch (event.user.code) {
                        ...
                        case EventUserTypeStep:
                            // not needed anymore: fixed-timestep handles updates
                            break;
                    }
                    break;
            }
